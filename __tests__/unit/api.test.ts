...
const u05: [string, () => void][] = [
  ...
  ['TC-U-133 every scheme status is valid enum value', () => { const v=['active','inactive','upcoming']; schemes.forEach(s => ok(v.includes(s.status))); }],
  ...
  ['TC-U-145 scheme deadlines are in 2026 or later', () => { schemes.forEach(s => ok(new Date(s.deadline).getFullYear() >= 2026)); }],
